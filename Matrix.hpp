...
private:
    ...
    constexpr typename std::enable_if<(_M == _N), void>::type lup_decompose() const {
        Matrix<T, M, N> A {*this};
...
public:
    ...
    template <typename _T = T, std::size_t _M = M, std::size_t _N = N>
    static typename std::enable_if<_M == _N, Matrix<_T, _M, _N>>::type identity() {
    ...
    template <typename _T = T, std::size_t _M = M, std::size_t _N = N>
    constexpr typename std::enable_if<(_M == _N) && (_M == 1), _T>::type determinant() {
        return (*this)(0, 0);
    }


    template <typename _T = T, std::size_t _M = M, std::size_t _N = N>
    constexpr typename std::enable_if<(_M == _N) && (_M == 2), _T>::type determinant() {
    ...
    template <typename _T = T, std::size_t _M = M, std::size_t _N = N>
    constexpr typename std::enable_if<(_M == _N), std::shared_ptr<Matrix<T, M, N>>>::type lower() const {
    ...
    template <typename _T = T, std::size_t _M = M, std::size_t _N = N>
    constexpr typename std::enable_if<(_M == _N), std::shared_ptr<Matrix<T, M, N>>>::type upper() const {
        if (!lup_decomposition_cache_valid_) lup_decompose();
        return upper_;
    }
    ...
    template <typename _T = T, std::size_t _M = M, std::size_t _N = N>
    constexpr typename std::enable_if<(_M == _N), std::array<std::size_t, M>>::type permutation() const {
        if (!lup_decomposition_cache_valid_) lup_decompose();
        return permutation_;
    }
    ...
    template <typename _T = T, std::size_t _M = M, std::size_t _N = N>
    constexpr typename std::enable_if<(_M == _N), std::tuple<std::shared_ptr<Matrix<T, M, N>>, std::shared_ptr<Matrix<T, M, N>>, std::array<std::size_t, M>>> lup_decomposition() const {
        if (!lup_decomposition_cache_valid_) lup_decompose();
    ...
    template <typename _T = T, std::size_t _M = M, std::size_t _N = N>
    constexpr typename std::enable_if<(_M == _N), Matrix<_T, _M, 1>>::type lup_solve(Matrix<_T, _M, 1>& b) const {
        if (!lup_decomposition_cache_valid_) lup_decompose();
        Matrix<_T, _M, 1> x;
        Matrix<_T, _M, 1> y;
        for (std::size_t i = 0; i < _M; ++i)
